...
- [~] **P1. The architecture ablations already listed in v0.5** — tied embeddings, RoPE, RMSNorm,
  ...
  Tied embeddings, RoPE, and RMSNorm are implemented behind independent architecture flags with
  persistence and source-level test coverage. Review, bundled execution, comparison against the
  seed floor, and the remaining GELU/SwiGLU implementation are still pending.
